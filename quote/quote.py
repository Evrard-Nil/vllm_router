...
def _collect_gpu_evidence(nonce_hex: str, no_gpu_mode: bool) -> list:
    if no_gpu_mode:
        logger.info("GPU evidence no-GPU mode enabled; using canned evidence")
    ...
    try:
        ...
        logger.error("NVML error while collecting GPU evidence: %s", error)
        raise Exception("NVML error during GPU evidence collection") from error
    except Exception as error:
        logger.error("GPU evidence collection failed: %s", error)
        raise
    finally:
        try:
            pynvml.nvmlShutdown()
